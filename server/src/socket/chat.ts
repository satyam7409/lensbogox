...
const roomId = (dealId: string) => String(dealId);

const serializeMessage = (doc: InstanceType<typeof Message>) => {
...
export const registerChatHandlers = (io: Server, socket: Socket) => {
  socket.on("join_room", async ({ dealId }, callback) => {
    try {
      ...
      const isParticipant =
        deal.hostUserId.toString() === socket.userId ||
  ...
  socket.on("leave_room", ({ dealId }) => {
    if (dealId) socket.leave(roomId(dealId));
  });
